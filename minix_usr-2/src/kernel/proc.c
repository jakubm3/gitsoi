...
FORWARD _PROTOTYPE( void user_insert_tail, (struct proc *rp) );
FORWARD _PROTOTYPE( struct proc *user_pick_next, (void) );
FORWARD _PROTOTYPE( void user_move_to_head, (struct proc *rp, struct proc *prev) );
...
PRIVATE void user_move_to_head(rp, prev)
struct proc *rp;
struct proc *prev;
{
  if (prev == NIL_PROC) return;
  prev->p_nextready = rp->p_nextready;
  rp->p_nextready = rdy_head[USER_Q];
  rdy_head[USER_Q] = rp;
  if (rdy_tail[USER_Q] == rp) rdy_tail[USER_Q] = prev;
}

PRIVATE void user_insert_tail(rp)
struct proc *rp;
{
  struct proc *p;
  struct proc *last_same;
  int g;
  ...
  p = rdy_head[USER_Q];
  while (p != NIL_PROC) {
    if (p->group == g) last_same = p;
    p = p->p_nextready;
  }

  if (last_same != NIL_PROC) {
    rp->p_nextready = last_same->p_nextready;
    last_same->p_nextready = rp;
    if (rdy_tail[USER_Q] == last_same) rdy_tail[USER_Q] = rp;
    return;
  }
  ...
  if (g == 1) {
    p = rdy_head[USER_Q];
    while (p->p_nextready != NIL_PROC && p->p_nextready->group == 0) p = p->p_nextready;
    rp->p_nextready = p->p_nextready;
    p->p_nextready = rp;
    if (rp->p_nextready == NIL_PROC) rdy_tail[USER_Q] = rp;
    return;
  }
...
PRIVATE struct proc *user_pick_next()
{
  struct proc *p;
  struct proc *prev;
  struct proc *best;
  struct proc *best_prev;
  unsigned long best_age;
  long best_rem;

  if (rdy_head[USER_Q] == NIL_PROC) return NIL_PROC;

  if (rdy_head[USER_Q]->group == 0) return rdy_head[USER_Q];
  ...
  while (p != NIL_PROC) {
    if (p->group == 1) {
      if (best == NIL_PROC || p->age > best_age) {
        best = p;
        best_prev = prev;
        best_age = p->age;
      }
  ...
  if (best != NIL_PROC) {
    best->age = 0;
    if (best_prev != NIL_PROC) user_move_to_head(best, best_prev);
    return rdy_head[USER_Q];
  }

  best = NIL_PROC;
  best_prev = NIL_PROC;
  best_rem = 0x7FFFFFFFL;
  if (best == NIL_PROC || p->sjf_rem < best_rem)
  prev = NIL_PROC;
  p = rdy_head[USER_Q];
  while (p != NIL_PROC) {
    if (p->group == 2) {
...
PRIVATE void sched()
{
  ...
  if (rdy_head[USER_Q] == NIL_PROC) rdy_tail[USER_Q] = NIL_PROC;

  if (rp->group == 1) rp->age = 0;
