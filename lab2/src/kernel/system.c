...
FORWARD _PROTOTYPE( int do_setburst, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_SETBURST:        r = do_setburst(&m);    break;
	    default:		r = E_BAD_FCN;
	}
...
unsigned short quants[3]= {5,3,1};

struct proc * findProcDescriptor( int pid_number){
 struct proc *rp;
 for(rp = BEG_PROC_ADDR; rp < END_PROC_ADDR; rp++)
     if(rp->p_pid != 0 && rp->p_pid == pid_number)
       return rp;
 return NULL;
}
PRIVATE int do_setgroup(register message *m_ptr){
 ...
 if (rp->p_nr < 0) return EPERM;
 if (m_ptr->m1_i2 < 0 || m_ptr->m1_i2 > 2) return EINVAL;
