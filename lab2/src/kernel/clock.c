 ...
 */
  ...
  if (--sched_ticks == 0) {
	...
	/* reset quantum depending on the proc priority group */
 ...
 */
  ...
  if (--sched_ticks == 0) {
    ...
    if (rdy_head[USER_Q] != NIL_PROC && rdy_head[USER_Q]->group >= 0 && rdy_head[USER_Q]->group < 3) {
        sched_ticks = SCHED_RATE * quants[rdy_head[USER_Q]->group];  /* reset quantum based on group */
    } else {
        sched_ticks = SCHED_RATE;  /* default quantum */
    }
    prev_ptr = bill_ptr;  /* new previous process */
}
